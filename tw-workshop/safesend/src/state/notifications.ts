...
const APP = COPY.app.name;

const SMS_FOOTER = `Open your ${APP} app to review. We never send links or ask for codes.`;
...
function describe(args: BuildArgs): {
  ...
  switch (type) {
    case 'approval_requested':
      return {
        sms: `${APP}: ${SENDER} has asked to send ${amount} to ${transfer?.payee.timesPaid === 0 ? 'a new payee' : payeeName}. Flagged ${band}. ${SMS_FOOTER}`,
        subject: `${SENDER} needs your approval for ${amount}`,
        body: `${SENDER} has asked to send ${amount} to ${payeeName}.\n\nReference: ${ref}\nRisk band: ${band}\nReason given: "${transfer?.reasonText ?? ''}"\n\nOpen ${APP} to see the full safety check and decide. We will never ask you for a code or send you a link.`,
        push: `${SENDER}: ${amount} to ${payeeName} — ${band}. Tap to review.`,
      };
    case 'approved':
      return {
        sms: `${APP}: ${APPROVER} approved your payment of ${amount}. ${SMS_FOOTER}`,
    ...
    case 'rejected':
      return {
        sms: `${APP}: ${APPROVER} has stopped a payment of ${amount}. ${SMS_FOOTER}`,
    ...
    case 'info_requested':
      return {
        sms: `${APP}: ${APPROVER} has a question about your payment of ${amount}. ${SMS_FOOTER}`,
        subject: `${APPROVER} has a question about your payment`,
        body: `${APPROVER} asked: "${detail ?? ''}"\n\nOpen ${APP} to reply. Your payment of ${amount} is on hold until you do.`,
        push: `${APPROVER} asked a question about ${amount}.`,
      };
    case 'info_answered':
      return {
        sms: `${APP}: ${SENDER} has replied to your question. ${SMS_FOOTER}`,
    ...
    case 'hold_started':
      return {
        sms: `${APP}: a payment of ${amount} is approved and will send after a short wait. ${SMS_FOOTER}`,
    ...
    case 'sent':
      return {
        sms: `${APP}: a payment of ${amount} has been sent. ${SMS_FOOTER}`,
    ...
    case 'expired':
      return {
        sms: `${APP}: a payment of ${amount} expired without a decision. ${SMS_FOOTER}`,
    ...
    case 'blocked':
      return {
        sms: `${APP}: a payment of ${amount} was not sent. ${SMS_FOOTER}`,
    ...
    case 'cancelled':
      return {
        sms: `${APP}: a payment of ${amount} was cancelled. ${SMS_FOOTER}`,
    ...
    case 'settings_change_pending':
      return {
        sms: `${APP}: a change to your safety settings has been requested. ${SMS_FOOTER}`,
        subject: 'A change to your safety settings was requested',
        body: `${detail ?? 'A setting change was requested.'}\n\nIt takes effect in 24 hours. Open ${APP} if you want to cancel it.`,
        push: detail ?? 'A setting change was requested.',
      };
    case 'settings_change_applied':
      return {
        sms: `${APP}: a change to your safety settings has taken effect. ${SMS_FOOTER}`,
    ...
    case 'settings_change_cancelled':
      return {
        sms: `${APP}: a requested settings change was cancelled. ${SMS_FOOTER}`,
    ...
    case 'trusted_payee_added':
      return {
        sms: `${APP}: a payee was added to your trusted list. ${SMS_FOOTER}`,
        subject: 'A payee was added to your trusted list',
        body: `${detail ?? ''}\n\nPayments to them below your checking amount will no longer be checked. You can undo this at any time in "${COPY.sender.whoHelpsMe}".`,
        push: detail ?? 'A trusted payee was added.',
      };
    case 'trusted_payee_revoked':
      return {
        sms: `${APP}: a trusted payee was removed. ${SMS_FOOTER}`,
    ...
    case 'contact_removal_pending':
      return {
        sms: `${APP}: a change to who helps with payments has been requested. ${SMS_FOOTER}`,
